...
class ProductoDAO{
    ...
    public function Registrar_Producto(Producto $producto){
        ...
        $stmt1="INSERT INTO Producto VALUES (NULL,:Marca,:Modelo,:Fecha_Ingreso,:Cantidad,:Precio,:Codigo)";
    ...
    public function eliminar_Producto($idProducto){
        $data_Source=new DataSource();
        $stmt1 = "DELETE FROM Productos WHERE id = :idProducto";
        $Resultado=$data_Source->ejecutarActulizacion($stmt1,array(':idProducto'=>$idProducto));
        return $Resultado;
    }
    public function Ver_Producto_id($idProducto){
        $data_Source=new DataSource();
        $data_Table=$data_Source->ejecutarConsulta("SELECT * FROM Productos WHERE id = :idProducto",array(':idProducto'=>$idProducto));
        $Producto=null;
    ...
    public function Editar_Producto($producto){
        $data_Source=new DataSource();
        $stmt1="UPDATE Productos SET Marca=:Marca,Modelo=:Modelo,Fecha_Ingreso=:Fecha_Ingreso,Cantidad=:Cantidad,Precio=:Precio,Codigo=:Codigo
        WHERE id=:idProducto";
        $Resultado=$data_Source->ejecutarActulizacion($stmt1, array(
            ':Marca'=>$producto->getMarca(),  
            ':Modelo'=>$producto->getModelo(), 
